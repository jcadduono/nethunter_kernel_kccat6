 ...
 * $Id: dhd_buzzz.h 524235 2015-01-06 08:22:40Z $
 */
...
#define BUZZZ_FMT(ID, format) \
	buzzz_fmt_reg(BUZZZ_EVT__## ID, "\t" format);
...
typedef enum buzzz_evt_id
{
	...
	BUZZZ_EVT(DHD_BUS_MEMDUMP)
} buzzz_evt_id_t;

static inline void buzzz_fmt_init(void)
{
	...
	BUZZZ_FMT(DHD_FILLUP_IOCT_REQST_BGN,
		"{ dhd_fillup_ioct_reqst_ptrbased cmd<%u> transid<%u>")
	...
	BUZZZ_FMT(DHD_BUS_MEMDUMP, "dhd_schedule_memdump buf(pa)=%x bufsize=%u")
}


#define BUZZZ_LOG(ID, N, ARG...)    buzzz_log ##N(BUZZZ_EVT__ ##ID, ##ARG)

#else  /* ! BUZZZ_LOG_ENABLED */

#define buzzz_attach()              do { /* noop */ } while (0)
#define buzzz_detach()              do { /* noop */ } while (0)
#define buzzz_panic(x)              do { /* noop */ } while (0)
